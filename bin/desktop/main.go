...
func main() {
	...
	// NOTE: We deliberately do NOT use StartState: WindowStateMaximised here.
	// On macOS, Wails applies StartState *before* it sets the window's initial
	// position, so an immediate maximise gets clobbered by the position reset
	// right after - leaving the window un-maximised (and its internal
	// "zoomed" flag out of sync, which also breaks the titlebar maximise
	// button/icon). Maximising once the window has actually been shown avoids
	// that race.
	var maximiseOnce sync.Once
	window.OnWindowEvent(events.Common.WindowShow, func(_ *application.WindowEvent) {
		maximiseOnce.Do(func() { window.Maximise() })
	})
